...
cam_name_map: dict[int, str] = {c["id"]: c["name"] for c in CAMERAS}
...
cam_active_tracks: dict[int, list] = {cid: [] for cid in CAM_IDS}
    ...
    while not stop_event.is_set():
        ...
        while not stop_event.is_set():
            ...
            if q is not None:
                ...
                # Đóng gói cả frame gốc (để save video k bị resize) và frame_resized (để YOLO)
                q.put_nowait((frame, frame_resized))
    ...
    while True:
        ...
        for cid, _ in cams_with_frames:
            ...
            if q is None:
                continue
            try:
                orig_f, res_f = q.get_nowait()
                batch_frames_orig.append(orig_f)
                batch_frames.append(res_f)
        ...
        # ── Xử lý Logic Scale Video 2x2 + Tracker phân phối về đúng cam ──
        with state_lock:
            for i, cid in enumerate(batch_cam_ids):
                orig_f = batch_frames_orig[i]
                
                # Resize về 1920x1080 chuẩn như scale_video_2
                frame_proc = cv2.resize(orig_f, (1920, 1080))
                h, w = frame_proc.shape[:2]
                h = h - (h % 2)
                w = w - (w % 2)
                frame_proc = frame_proc[:h, :w]
                ch, cw = h // 2, w // 2
                
                margin = 140
                bbox_ranges = [
                    (0, min(w, cw + margin), 0, min(h, ch + margin)),
                    (max(0, cw - margin), w, 0, min(h, ch + margin)),
                    (0, min(w, cw + margin), max(0, ch - margin), h),
                    (max(0, cw - margin), w, max(0, ch - margin), h)
                ]
                
                crops = []
                offsets = []
                for (x1, x2, y1, y2) in bbox_ranges:
                    crops.append(frame_proc[y1:y2, x1:x2])
                    offsets.append((x1, y1))
                    
                temp_boxes = []
                # Inference tuần tự từng crop (Do BATCH_SIZE của tensorRT đang là 1)
                for c_idx, crop in enumerate(crops):
                    with torch.no_grad():
                        res = local_model.predict(source=crop, device=DEVICE, imgsz=640, classes=[PERSON_CLASS], conf=CONF_THRESH, verbose=False)[0]
                    offset_x, offset_y = offsets[c_idx]
                    if res.boxes is not None and len(res.boxes) > 0:
                        for box in res.boxes:
                            coords = box.xyxy[0].cpu().numpy()
                            cx1_, cy1_, cx2_, cy2_ = coords
                            ox1 = int(cx1_ + offset_x)
                            oy1 = int(cy1_ + offset_y)
                            ox2 = int(cx2_ + offset_x)
                            oy2 = int(cy2_ + offset_y)
                            conf = float(box.conf)
                    ...
                    for b_new in temp_boxes:
                        ...
                        for k_box in final_boxes:
                            ...
                            if x2_inter > x1_inter and y2_inter > y1_inter:
                                ...
                                ioa = inter_area / min_area if min_area > 0 else 0
                ...
                active_tracks = cam_active_tracks.get(cid, [])
                new_tracks = []
                ...
                for track in active_tracks:
                    ...
                    for idx, box in enumerate(unmatched_boxes):
                        ...
                        cx1_o = (track['box'][0] + track['box'][2]) / 2
                        cy1_o = (track['box'][1] + track['box'][3]) / 2
                        cx2_o = (box[0] + box[2]) / 2
                        cy2_o = (box[1] + box[3]) / 2
                        dist = ((cx1_o - cx2_o)**2 + (cy1_o - cy2_o)**2)**0.5
                ...
                cam_active_tracks[cid] = new_tracks
                confirmed_boxes = [t['box'] for t in new_tracks if t.get('is_moving', False) or t['hits'] >= 5]
                raw_person = len(confirmed_boxes)
                ...
                # Trigger recording if stable_count > 5
                rec_state = recording_state.get(cid)
                if rec_state is not None:
                    if stable_count > 5 and not rec_state["is_recording"] and (now - rec_state["last_trigger_time"] > RECORD_COOLDOWN):
                        rec_state["is_recording"] = True
                        rec_state["frames"] = []
                        rec_state["people_count"] = stable_count
                        rec_state["last_trigger_time"] = now
                        print(f"🎥 Bắt đầu ghi video cam {cid} ({stable_count} người)")
                    
                    if rec_state["is_recording"]:
                        rec_state["frames"].append(batch_frames_orig[i])
                        if len(rec_state["frames"]) >= 20: # 4 seconds @ 5 fps
                            cam_name = cam_name_map.get(cid, str(cid))
                            video_write_queue.put({
                                "cam_name": cam_name,
                                "frames": list(rec_state["frames"]),
                                "people_count": rec_state["people_count"]
                            })
                ...
                brightness = get_brightness(batch_frames_orig[i])
...
def video_writer_worker():
    while True:
        try:
            ...
            # format: tên_cam_thời_gian_số_người_ngày
            sanitized_name = "".join([c for c in cam_name if c.isalnum() or c in " -_"])
            filename = f"{sanitized_name}_{time_str}_{people_count}_{date_str}.mp4"
            filepath = os.path.join("saved_videos", filename)
            
            if frames:
                h, w = frames[0].shape[:2]
                fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                out = cv2.VideoWriter(filepath, fourcc, 5.0, (w, h))
                for f in frames:
                    out.write(f)
                out.release()
    ...
    while True:
        ...
        for cid in removed:
            ...
            with state_lock:
                ...
                cam_active_tracks.pop(cid, None)
                last_detect_time.pop(cid, None)
                cam_topic_map.pop(cid, None)
                cam_name_map.pop(cid, None)
                cam_ip_map.pop(cid, None)
                recording_state.pop(cid, None)
