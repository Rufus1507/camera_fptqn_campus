...
def rtsp_worker(cam: dict, stop_event: threading.Event):
    ...
    while not stop_event.is_set():
        ...
        while not stop_event.is_set():
            ...
            # Hạ kích thước khung hình về chuẩn chung 1920x1080 (Full HD) trước khi crop 2x2
            if skip_resize:
                frame_resized = frame
            else:
...
def _yolo_thread_logic():
    ...
    while True:
        ...
        # Giới hạn thời gian bật model AI: 5 FPS (200ms) tựa như bên scale_video_2.py
        time.sleep(0.20)

        # ── GPU Inference (Lần lượt từng crop của batch) ─────────────────────
        with torch.no_grad():
            for i, frame in enumerate(batch_frames):
                cid = batch_cam_ids[i]
                
                # Cắt frame thành 4 phần (lưới 2x2)
                h_img, w_img = frame.shape[:2]
                h_img = h_img - (h_img % 2)
                w_img = w_img - (w_img % 2)
                frame_cropped = frame[:h_img, :w_img]
                ch, cw = h_img // 2, w_img // 2
                
                margin = 140
                bbox_ranges = [
                    (0, min(w_img, cw + margin), 0, min(h_img, ch + margin)), # Top-Left
                    (max(0, cw - margin), w_img, 0, min(h_img, ch + margin)), # Top-Right
                    (0, min(w_img, cw + margin), max(0, ch - margin), h_img), # Bottom-Left
                    (max(0, cw - margin), w_img, max(0, ch - margin), h_img)  # Bottom-Right
                ]
                
                crops = []
                offsets = []
                for (x1, x2, y1, y2) in bbox_ranges:
                    crops.append(frame_cropped[y1:y2, x1:x2])
                    offsets.append((x1, y1))
                    
                temp_boxes = []
                # Chạy inference lần lượt từng crop (batch=1) để tránh tràn VRAM
                for j, crop in enumerate(crops):
                    res = local_model.predict(
                        source=crop,
                        device=DEVICE,
                        imgsz=RESIZE[0],
                    ...
                    if res.boxes is not None and len(res.boxes) > 0:
                        for box in res.boxes:
                            ...
                            temp_boxes.append([ox1, oy1, ox2, oy2, conf_val])
                            
                final_boxes = []
                # Thuật toán NMS khử vùng giao nhau
                if len(temp_boxes) > 0:
